...
function printInvoice($frm, $printerName, $tienePropina = false) {
    ...
    */
    ...
    $printer->setEmphasis(true);
    $printer->setTextSize(2,1);
    $printer->text("Encuesta de Satisfaccion \n");
	$printer->setEmphasis(false);
    $printer->setTextSize(1,1);
	$printer->setJustification(Printer::JUSTIFY_LEFT);
    $printer->text("Hola, estamos interesados en ofrecer siempre \n");
	$printer->text("calidad en nuestro servicio. Por favor ayudanos con 3 preguntas. \n");
	$printer->text("\n");
   
	$printer->text("* Como califica la atencion del Asesor de caja? \n");
